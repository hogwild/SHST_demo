...
function [S L seg seg_vals seg_lab_vals seg_edges] = msseg(img,lab_vals,hs,hr,M,full)
  ...
  S = fimg; %Luv2RGB(fimg); 
  L = labels + 1; % the size of L is same to the image
  ...
  if full == 1,
      ...
      [points edges]=lattice(X,Y,0);    clear points;  %%create the edges for the image, each pixel is a node
      d_edges = edges(find(L(edges(:,1))~=L(edges(:,2))),:); %% edge(:,1) is the position of a node and edge(:,2) is the node next to it, first from the direction of x coordinate and then the direction of y coordinate. 
      all_seg_edges = [L(d_edges(:,1)) L(d_edges(:,2))]; all_seg_edges = sort(all_seg_edges,2); 
  
      tmp = zeros(nseg,nseg);
      tmp(nseg*(all_seg_edges(:,1)-1)+all_seg_edges(:,2)) = 1;
      [edges_x edges_y] = find(tmp==1); seg_edges = [edges_x edges_y];%%the 'seg_edges' contains the edges between the superpixels.i.e. it tells which two superpixels are connected. 
  end;
