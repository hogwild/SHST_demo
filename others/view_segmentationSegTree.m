function view_segmentationSegTree(img,labels,out_path,only_name,lifetimelevel,display)

[X Y Z] = size(img); L = max(labels); imwrite(img,fullfile(out_path, [only_name '.bmp']));

final_path = fullfile(out_path,['LifetimeOrNSegs_',int2str(lifetimelevel)]);
