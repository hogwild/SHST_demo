function [label_img, clusterNumber] = getLabel_with_specified_cluster_number(SegTree,ic,parentId,theKeptId,img_size,Nsegs)%IdxOfKeptRegion,IdxOfRemovedRegion,IdxOfMergedTo,img_size,threshold) 
...
edgeWeights = unique(edgeWeights);
if Nsegs>length(edgeWeights)
    %threshold=edgeWeights(end)+1;
    clusterNumber = size(SegTree,1);
    label=1:clusterNumber;
...
% else
%     selectlifetime = sortedlifetime(Nsegs);
%     threshold = edgeWeights(lifetime==selectlifetime);
%     threshold = max(threshold);% to avoid there are more than one threshold;
% end
